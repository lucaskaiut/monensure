...
interface SearchSelectProps {
  ...
  onCreateOption?: (search: string) => Promise<SearchSelectOption | null>
...
export function SearchSelect({
  ...
  resolveLabel,
  onCreateOption,
  createOptionLabel = 'Criar registro',
  label,
  hint,
  error,
  required,
  ...
  const showCreateOption = Boolean(onCreateOption && trimmedInput && !loading && options.length === 0)
  const selectableCount = options.length + (showCreateOption ? 1 : 0)
  ...
  const handleCreate = async () => {
    if (!onCreateOption || creating || !trimmedInput) {
      return
    }
    ...
    try {
      const option = await onCreateOption(trimmedInput)
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'ArrowDown') {
      event.preventDefault()
      setHighlight((current) => Math.min(current + 1, selectableCount - 1))
      ...
      if (showCreateOption && highlight === options.length) {
        void handleCreate()
        return
      }
            ...
            {showCreateOption && (
              <li role="option" aria-selected={false}>
                <button
                  ...
                  className={cn(
                    'flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-left text-sm transition-colors disabled:cursor-not-allowed disabled:opacity-60',
                    options.length === highlight ? 'bg-surface-2' : 'text-foreground',
                  )}
                >
                  {creating ? (
                  ...
                  <span className="min-w-0 truncate">
                    <span className="font-medium text-primary">{createOptionLabel}</span>
                    <span className="text-muted"> — {trimmedInput}</span>
                  </span>
                </button>
              </li>
            )}
