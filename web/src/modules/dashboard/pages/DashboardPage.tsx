...
export default function DashboardPage() {
  ...
  const canReadPayables = can(Permission.PAYABLE_READ)
  const financialSummary = useFinancialSummary(canReadPayables)
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <FinancialCard
                ...
                icon={AlertTriangle}
                metric={financialSummary.data?.overdue}
                loading={financialSummary.isPending}
              ...
              <FinancialCard
                label="Vencendo hoje"
                icon={CalendarCheck}
                metric={financialSummary.data?.due_today}
                loading={financialSummary.isPending}
              ...
              <FinancialCard
                ...
                icon={CalendarDays}
                metric={financialSummary.data?.next_7_days}
                loading={financialSummary.isPending}
                to="/financial/payables"
                accent="bg-primary-soft text-primary"
              />
              <FinancialCard
                ...
                icon={CalendarDays}
                metric={financialSummary.data?.next_30_days}
                loading={financialSummary.isPending}
                to="/financial/payables"
                accent="bg-primary-soft text-primary"
              />
              <FinancialCard
                ...
                icon={Wallet}
                total={financialSummary.data?.open_total}
                loading={financialSummary.isPending}
                to="/financial/payables"
                accent="bg-success-soft text-success"
              />
...
function FinancialCard({
  ...
  const value = total ?? metric?.total
      ...
      <Link to={to} className="block rounded-xl">
        <CardContent className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            {metric && !loading && (
              <p className="text-[13px] text-muted">{metric.count} conta(s)</p>
            )}
          </div>
        </CardContent>
      </Link>
