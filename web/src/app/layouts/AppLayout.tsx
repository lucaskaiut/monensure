...
import { Building2, CalendarClock, CreditCard, FolderTree, LayoutDashboard, KeyRound, LogOut, Menu, Receipt, ScrollText, Settings, ShieldCheck, Sparkles, TrendingUp, Truck, Users, Wallet, Webhook, Zap } from 'lucide-react'
  ...
  return (
    <Sidebar header={<Brand />}>
      <SidebarGroup label="Geral">
        ...
        {can(Permission.ASSISTANT_MANAGE) && (
          <SidebarItem
            to="/assistant/settings"
            icon={Settings}
            label="Config. do assistente"
      ...
      {isOperatingTenant && (
        <SidebarGroup label="Financeiro">
          {can(Permission.PAYABLE_READ) && (
            <SidebarItem to="/financial/payables" icon={Wallet} label="Contas a pagar" onNavigate={onNavigate} />
          )}
          {can(Permission.CASHFLOW_READ) && (
            <SidebarItem to="/financial/cashflow" icon={TrendingUp} label="Fluxo de caixa" onNavigate={onNavigate} />
          )}
          {can(Permission.SUPPLIER_READ) && (
            <SidebarItem to="/financial/suppliers" icon={Truck} label="Fornecedores" onNavigate={onNavigate} />
          )}
          {can(Permission.CATEGORY_READ) && (
            <SidebarItem to="/financial/categories" icon={FolderTree} label="Categorias" onNavigate={onNavigate} />
          )}
          {can(Permission.RECURRENCE_READ) && (
            <SidebarItem to="/financial/recurrences" icon={CalendarClock} label="Recorrências" onNavigate={onNavigate} />
          )}
        </SidebarGroup>
      )}
