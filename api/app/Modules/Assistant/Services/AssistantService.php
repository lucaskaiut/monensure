...
final class AssistantService
{
    ...
    private function buildSystemPrompt(AssistantAgent $agent): string
    {
        $system = $agent->systemPrompt();
        $additional = $this->settings->additionalPrompt();
        ...
        try {
            for ($i = 0; $i < $maxIterations; $i++) {
                ...
                $result = $this->client->streamChat(
                    ...
                    $this->logContext($conversation, $iteration),
                );
            ...
            Log::warning('Assistente de IA: limite de iterações de tools atingido', array_merge(
                $this->logContext($conversation, $iteration),
            ...
            Log::warning('Assistente de IA: falha durante o chat', array_merge(
                $this->logContext($conversation, $iteration),
                [
                    'model' => $config['model'],
    ...
    /**
     * @return array{conversation_id: int|string, conversation_uuid: string, tenant_id: int|string|null, iteration: int}
     */
    private function logContext(Conversation $conversation, int $iteration): array
    {
        return [
