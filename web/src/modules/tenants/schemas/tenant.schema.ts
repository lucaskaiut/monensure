...
const accessFields = {
  ...
  complimentary_ends_at: z.string().nullable(),
}

export const createChildTenantSchema = z
  .object({
    tenant: tenantFields,
    user: z.object({
      name: z.string().min(1, 'Informe o nome'),
  ...
  .superRefine((values, ctx) => {
    if (values.is_complimentary && !values.plan_id) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['plan_id'],
        message: 'Selecione um plano para liberar o acesso cortesia.',
      })
    }
  })

export const updateChildTenantSchema = z
  .object({
    tenant: tenantFields,
    ...accessFields,
  })
